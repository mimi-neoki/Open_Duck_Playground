...
from joystick_chatgpt import ChatGPTJoystick
import threading
import queue
...
class MjInferGPT(MJInferBase):
    def __init__(
        self, model_path: str, reference_data: str, onnx_model_path: str, standing: bool
    ):
        super().__init__(model_path)

        self.chat = ChatGPTJoystick(model="gpt-4o-mini")
        self.prompt = "Move forward slowly"
        self._chat_interval = 0.5   # [s] = 2 Hz で問い合わせ
        self._t_next_chat = time.time()

        self._chat_interval = 0.5                # 2 Hz 更新
        self._prompt_q: queue.Queue[str] = queue.Queue(maxsize=1)
        self._cmd_q:    queue.Queue[np.ndarray] = queue.Queue(maxsize=1)
        threading.Thread(target=self._chat_worker, daemon=True).start()
        threading.Thread(target=self._cli_prompt_loop, daemon=True).start()
        ...
        # threading.Thread(
        #     target=lambda: [setattr(self, "prompt", input(">>> ").strip())
        #                     for _ in iter(int, 1)],
        #     daemon=True
        # ).start()        
    ...
    def _chat_worker(self):
        while True:
            now = time.time()
            if now >= self._t_next_chat:
                try:
                    # 最新プロンプトを取得（なければ既存を使う）
                    self.prompt = self._prompt_q.get_nowait()
                except queue.Empty:
                    pass
                cmd = self.chat.ask(self.prompt)
                if not self._cmd_q.empty():
                    self._cmd_q.get_nowait()
                self._cmd_q.put(cmd)
                self._t_next_chat = now + self._chat_interval
            time.sleep(0.01)        # CPU 暴走防止

    def _cli_prompt_loop(self):
        """ターミナルから自然言語指示を受け取りキューへ流す。"""
        while True:
            try:
                line = input(">>> ").strip()
            ...
            with mujoco.viewer.launch_passive(
                ...
                while True:
                    ...
                    if counter % self.decimation == 0:
                        ...
                        # if time.time() >= self._t_next_chat:
                        #     self.commands = self.chat.ask(self.prompt)
                        #     self._t_next_chat += self._chat_interval
                        try:
                            self.commands = self._cmd_q.get_nowait()
                        except queue.Empty:
                            pass     # 前回の commands をそのまま使う
